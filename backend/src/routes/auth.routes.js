...
const passwordMsg = "Min 8 chars, 1 uppercase, 1 lowercase, 1 number, 1 special character";
...
function setAuthCookies(res, user) {
  ...
  res.cookie(REFRESH_COOKIE_NAME, refreshToken, cookieOpts(30 * 24 * 60 * 60 * 1000));
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!code) return res.redirect(`${process.env.CLIENT_URL}/login?error=google_failed`);
...
const changePasswordSchema = z.object({
  ...
  newPassword: z.string().regex(passwordRegex, passwordMsg),
});
